...
import PluginReference.ChatColor;import PluginReference.MC_Command;import PluginReference.MC_ItemStack;import PluginReference.MC_Player;import org.projectrainbow._DiwUtils;import org.projectrainbow._EconomyManager;import java.util.List;public class _CmdSetWorth implements MC_Command{
...
}else if (plr == null){_DiwUtils.ConsoleMsg("Not from console!  Requires item in hand.");
}else{double val = 0.0D;
String strAmt = args[0];
try{val = Double.parseDouble(strAmt);
}catch (Exception var9){_DiwUtils.ConsoleMsg("Invalid amount: " + strAmt);
return;}if (val <= 0.0D){
val = 0.0D;}if (plr.getItemInHand() == null){
