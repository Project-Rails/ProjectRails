...
import PluginReference.ChatColor;import PluginReference.MC_Player;import com.google.common.io.Files;import joebkt._JOT_OnlineData;import joebkt._JOT_OnlineTimeEntry;import net.minecraft.entity.player.EntityPlayer;import org.projectrainbow.commands._CmdDiw;import java.io.BufferedInputStream;import java.io.BufferedOutputStream;import java.io.File;import java.io.FileInputStream;import java.io.FileOutputStream;import java.io.ObjectInputStream;import java.io.ObjectOutputStream;import java.util.ArrayList;import java.util.Collections;import java.util.UUID;public class _JOT_OnlineTimeUtils{
...
try{long exc = System.currentTimeMillis();
...
s.close();long msEnd = System.currentTimeMillis();
...
}catch (Throwable var10){var10.printStackTrace();
_DiwUtils.ConsoleMsg("Starting new file: " + DataFilename);
Data = new _JOT_OnlineData();
...
}catch (Throwable var8){System.out.println("**********************************************");
...
"b"));}entry = new _JOT_OnlineTimeEntry();
...
}else{entry.msLastLogout = System.currentTimeMillis();
...
try{for (MC_Player p:ServerWrapper.getInstance().getPlayers()){
HandlePlayerLogout(p.getName(), p.getUUID());
}}catch (Exception var3){}ShuttingDown = true;
...
}else if (!CanSeeHidden(cs) && Data.hidePlayer.containsKey(uuid)){_DiwUtils.ConsoleMsg(
...
+ pname);reply(cs, ChatColor.RED + "No data for: " + ChatColor.YELLOW + pname);
}else{reply(cs,ChatColor.AQUA
+ "----------------------------------------------------");
byte labelLen = 20;
reply(cs,ChatColor.AQUA
...
reply(cs,"" + ChatColor.AQUA + ChatColor.BOLD
+ "----- Online Time Options -----");
reply(cs,ChatColor.GOLD + "/jot PlayerName" + ChatColor.WHITE
+ " -- Show player online time.");
reply(cs,ChatColor.GOLD + "/jot top [Page#]" + ChatColor.WHITE
+ " -- Show top online time.");
if (HasAdminPerm(cs)){
String pref = ChatColor.RED + "[Admin] ";
reply(cs,pref + ChatColor.LIGHT_PURPLE + "/jot " + ChatColor.AQUA
+ "[hide|unhide] " + ChatColor.YELLOW + "PlayerName");
reply(cs,pref + ChatColor.LIGHT_PURPLE + "/jot " + ChatColor.AQUA
+ "listhide " + ChatColor.WHITE + " -- List who is hidden.");
}reply(cs,"" + ChatColor.DARK_AQUA + ChatColor.ITALIC
...
name1));long total2 = _JOT_OnlineTimeUtils.GetTotalOnlineTime(
_JOT_OnlineTimeUtils.Data.playerData.get(
name2));return total1 > total2 ? -1 : (total1 < total2 ? 1 : 0);
...
reply(cs,ChatColor.GOLD + "Showing " + ChatColor.WHITE + var13
+ ChatColor.GOLD + ", records " + ChatColor.WHITE
+ String.format("%d - %d",
idxStart + 1,
idxEnd));for (int i = idxStart; i < idxEnd; ++i){
...
+ _DiwUtils.TextLabel(
String.format("#%d ",
i + 1)+ name,20)+ " "+ ChatColor.WHITE
+ _DiwUtils.TimeDeltaString(
GetTotalOnlineTime(entry))
+ trailer;if (!(cs instanceof EntityPlayer)){
line = ChatColor.AQUA
+ String.format(
...
reply(cs,ChatColor.LIGHT_PURPLE + "Warning: No online time yet for "
+ ChatColor.YELLOW + name);
exactName = name;
}Data.hidePlayer.put(exactName, flag);
if (!flag){Data.hidePlayer.remove(exactName);
}reply(cs,ChatColor.GREEN + "Set \'online time visible\' flag for "
...
reply(cs,ChatColor.GREEN + "Names Hidden: " + ChatColor.YELLOW
+ page1);return true;
}else if (HasAdminPerm(cs) && args.length == 1&& args[0].equalsIgnoreCase("save")){
SaveData();reply(cs, ChatColor.GREEN + "Data Saved.");
return true;
}else if (args.length >= 1 && args[0].equalsIgnoreCase("top")){int page = 1;
try{page = Integer.parseInt(args[1]);
}catch (Exception var4){}if (page < 1){
page = 1;}ShowTopPage(cs, page);
return true;
}else if (args.length == 1){ShowPlayerLoginTime(cs, args[0]);
return true;
}else if (HasAdminPerm(cs) && args.length == 2&& args[0].equalsIgnoreCase("hide")){
SetPlayerHide(cs, args[1], true);
return true;
}else if (HasAdminPerm(cs) && args.length == 2&& args[0].equalsIgnoreCase("unhide")){
