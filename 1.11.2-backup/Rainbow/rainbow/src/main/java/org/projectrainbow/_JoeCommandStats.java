...
import java.util.HashMap;import java.util.concurrent.ConcurrentHashMap;public class _JoeCommandStats{
...
}else{String res = stats.JoeNewCommand(msg, curMS, pName);
if (res != null){
return res;}cmdHist.put(pName, stats);
}return null;
...
if (msg == null){
return null;
}else{String[] skips = new String[]{
"help", "plot", "worth", "sell",
"buy", "ec", "bp", "worth"};String msgLower = msg.toLowerCase();
...
msg);if (this.lastMessage.equals(msg.toUpperCase())
&& msg.length() > 4
&& (msg.charAt(0) != 47 || msg.startsWith("/msg ")
|| msg.startsWith("/r "))){
String logmsg = String.format(
"*** SPAM KICK: Repeated CAPS by %s: %s",
new Object[]{pName, msg});System.out.println(
"--------------------------------------------");
...
/ 1000.0D);if (this.count >= 4){
if (this.count <= 5 && var13 >= 1.0D){
var12 = true;
}else if (this.count > 5 && this.count <= 7&& var13 >= 0.66D){
var12 = true;
}else if (this.count > 7 && var13 >= 0.33D){var12 = true;
}if (var12){String logmsg1 = String.format(
"*** SPAM KICK: %s: %.2f cmd/sec. n=%d : %s",
new Object[]{
pName, Double.valueOf(var13),
Integer.valueOf(this.count), msg});System.out.println(
"--------------------------------------------");
