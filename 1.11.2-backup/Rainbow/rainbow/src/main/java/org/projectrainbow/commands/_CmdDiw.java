...
import PluginReference.ChatColor;import PluginReference.MC_Player;import PluginReference.RainbowUtils;import net.minecraft.command.CommandBase;import net.minecraft.command.CommandException;import net.minecraft.command.ICommandManager;import net.minecraft.command.ICommandSender;import net.minecraft.entity.player.EntityPlayer;import net.minecraft.entity.player.EntityPlayerMP;import net.minecraft.init.Blocks;import net.minecraft.server.MinecraftServer;import net.minecraft.util.math.BlockPos;import org.projectrainbow._ColorHelper;import org.projectrainbow._DiwUtils;import org.projectrainbow._Janitor;import org.projectrainbow._UUIDMapper;import org.projectrainbow.interfaces.IMixinEntityPlayerMP;import org.projectrainbow.interfaces.IMixinICommandSender;import org.projectrainbow.interfaces.IMixinPlayerCapabilities;import java.io.*;import java.util.Arrays;import java.util.Collections;import java.util.List;import java.util.UUID;import static org.projectrainbow.launch.Bootstrap.args;public class _CmdDiw extends CommandBase{
@Overridepublic String getCommandName(){
return "diw";
}@Overridepublic boolean checkPermission(MinecraftServer minecraftServer, ICommandSender cs){
...
}else{p.getEntityWorld().setBlockState(new BlockPos(x + dx, y + dy, z + dz), Blocks.DIRT.getDefaultState(), 3);
...
int z = (int) p.lastTickPosZ;
for (int dx = -1 * radius; dx <= radius; ++dx){
int dy = 0;p.getEntityWorld().setBlockState(new BlockPos(x + dx, y + dy, z + radius), Blocks.COBBLESTONE_WALL.getDefaultState(), 3);
int dz = -1 * radius;
p.getEntityWorld().setBlockState(new BlockPos(x + dx, y + dy, z + dz), Blocks.COBBLESTONE_WALL.getDefaultState(), 3);
}for (int dz = -1 * radius; dz <= radius; ++dz){
int dy = 0;p.getEntityWorld().setBlockState(new BlockPos(x + radius, y + dy, z + dz), Blocks.COBBLESTONE_WALL.getDefaultState(), 3);
int dx = -1 * radius;
p.getEntityWorld().setBlockState(new BlockPos(x + dx, y + dy, z + dz), Blocks.COBBLESTONE_WALL.getDefaultState(), 3);
...
if (!lwr.startsWith(";")){
try{for (int i = 0; i < 10 && line.indexOf(36) >= 0; ++i){
...
}catch (Exception var15){System.out.println("[Script] Command Error: " + var15.getMessage());
}}}}br.close();}catch (Exception var16){var16.printStackTrace();
}}@Overridepublic String getCommandUsage(ICommandSender cs){
...
String[] arrCmds = new String[]{"save", "speed", "flyspeed", "walkspeed", "mem", "skyclear", "setgrass", "border", "echo", "script", "loadBanList", "clean", "namecolor", "loadconfig", "flow", "bp", "ec"};List<String> cmds = Arrays.asList(arrCmds);
...
this.ShowUsage(cs);
}else if (args[0].equalsIgnoreCase("loadBanList")){try{minecraftServer.getPlayerList().getBannedPlayers().readSavedFile();
...
}}else if (args[0].equalsIgnoreCase("namecolor")){if (args.length < 3){
_DiwUtils.reply(cs, _ColorHelper.RED + "Usage: /diw namecolor <User> <new nickname|off>");
}else{UUID uuid = _UUIDMapper.getUUID(args[1]);
if (uuid == null){
_DiwUtils.reply(cs, _ColorHelper.RED + "Player not found.");
}else{String newName = _DiwUtils.ConcatArgs(args, 2);
...
}else{_CmdNameColor.ColorNameDict.put(uuid.toString(), newName);
...
}}}}else if (args[0].equalsIgnoreCase("flow")){_DiwUtils.BlockFlowOn = !_DiwUtils.BlockFlowOn;
_DiwUtils.reply(
cs,ChatColor.GREEN
+ "Joe\'s BlockFlowOn: "
+ ChatColor.WHITE
+ _DiwUtils.BlockFlowOn);
}else if (args[0].equalsIgnoreCase("loadconfig")){_DiwUtils.LoadRainbowProperties();
_DiwUtils.reply(cs,
ChatColor.GREEN
+ "Configuration reloaded: "
+ ChatColor.AQUA
+ _DiwUtils.RainbowPropertiesFilename);
}else if (args[0].equalsIgnoreCase("save")){_DiwUtils.SaveStuffs();
_DiwUtils.reply(cs, _ColorHelper.GREEN + "Rainbow data saved.");
}else if (args[0].equalsIgnoreCase("flyspeed") && p != null){if (args.length < 2){
((IMixinICommandSender) p).sendMessage(_ColorHelper.RED + "Example 1: " + _ColorHelper.GOLD + "/diw flySpeed 2");
}else{float spd = Float.parseFloat(args[1]);
...
}}else if (args[0].equalsIgnoreCase("speed") && p != null){((IMixinICommandSender) p).sendMessage(_ColorHelper.GOLD + String.format("Fly Speed: %.2f, Walk Speed: %.2f", (double) p.capabilities.getFlySpeed() / 0.05D, (double) p.capabilities.getWalkSpeed() / 0.1D));
...
}else if (args[0].equalsIgnoreCase("walkspeed") && p != null){if (args.length < 2){
((IMixinICommandSender) p).sendMessage(_ColorHelper.RED + "Example 1: " + _ColorHelper.GOLD + "/diw walkspeed 2");
}else{float spd = Float.parseFloat(args[1]) * 0.1F;
...
}}else if ((args[0].equalsIgnoreCase("setgrass") || args[0].equalsIgnoreCase("skyclear")) && p != null){int radius = 5;
try{radius = Integer.parseInt(args[1].trim());
}catch (Throwable var18){radius = 5;}if (radius < 0){
radius *= -1;
}if (radius >= 200){
radius = 200;
}if (args[0].equalsIgnoreCase("setgrass")){
this.HandleDiwGrass(p, radius);
}else{this.HandleDiwSky(p, radius);
}}else if (args[0].equalsIgnoreCase("border") && p != null){int radius = 5;
try{radius = Integer.parseInt(args[1].trim());
}catch (Throwable var19){radius = 5;}if (radius < 0){
radius *= -1;
}if (radius >= 200){
radius = 200;
}this.HandleDiwBorder(p, radius);
}else if (args[0].equalsIgnoreCase("echo")){if (args.length <= 1){
_DiwUtils.reply(cs, _ColorHelper.RED + "Usage: /diw echo " + _ColorHelper.YELLOW + "your message to everyone.");
}else{String msg = _DiwUtils.ConcatArgs(args, 1);
msg = _DiwUtils.SpecialTranslate(msg);
msg = _DiwUtils.TranslateColorString(msg, true);
_DiwUtils.MessageAllPlayers(msg);
}}else if (args[0].equalsIgnoreCase("script")){if (args.length <= 1){
...
}else{String fname = _DiwUtils.RainbowDataDirectory + "Scripts" + File.separator + args[1];
if (!fname.endsWith(".txt")){
fname = fname + ".txt";
}File f = new File(fname);
if (!f.exists()){
_DiwUtils.reply(cs, _ColorHelper.RED + "File not found: " + _ColorHelper.YELLOW + fname);
}else{this.HandleScript(p, fname, args);
}}}else if (args[0].equals("clean")){if (args.length == 1){
...
_Janitor.DoMobClean(p, true,
new String[]{
"Zombie","Skeleton", "BatEntity", "Enderman",
"Witch", "Spider", "MagmaCube",
"Creeper", "Ghast", "ZombiePigman",
"Silverfish", "Slime", "CaveSpider",
"Blaze"});}else{_Janitor.DoMobClean(p, true,
new String[]{var35});}}}else if (args[0].equalsIgnoreCase("mem") || args[0].equalsIgnoreCase("jc")){Runtime rt = Runtime.getRuntime();
...
}else if (args[0].equalsIgnoreCase("autorestart")){if (_DiwUtils.g_restartCountdown >= 0L){
_DiwUtils.reply(cs, ChatColor.GREEN + "Auto-Restart cancelled!");
_DiwUtils.g_restartCountdown = -1L;
}else{_DiwUtils.reply(cs, ChatColor.GREEN + "Auto-Restart set for 70 seconds!");
_DiwUtils.g_restartCountdown = 70L;
}}else if (args[0].equalsIgnoreCase("bp") && p != null){if (args.length != 2){
_DiwUtils.reply(p, ChatColor.RED + "Usage: /diw bp " + ChatColor.YELLOW + "PlayerName");
}else{MC_Player other = RainbowUtils.getServer().getOfflinePlayerByName(args[1]);
if (other == null){
_DiwUtils.reply(p, ChatColor.RED + "No player found: " + ChatColor.YELLOW + args[1]);
}else{p.displayGUIChest(((IMixinEntityPlayerMP) other).getBackpack());
...
}}}else if (args[0].equalsIgnoreCase("ec") && p != null){if (args.length != 2){
_DiwUtils.reply(p, ChatColor.RED + "Usage: /diw ec " + ChatColor.YELLOW + "PlayerName");
}else{MC_Player other = RainbowUtils.getServer().getOfflinePlayerByName(args[1]);
if (other == null){
_DiwUtils.reply(p, ChatColor.RED + "No player found: " + ChatColor.YELLOW + args[1]);
}else{p.displayGUIChest(((EntityPlayerMP) other).getInventoryEnderChest());
...
}}}else{_DiwUtils.reply(cs, _ColorHelper.RED + "Unknown Option: " + _ColorHelper.AQUA + args[0]);
this.ShowUsage(cs);
}}}