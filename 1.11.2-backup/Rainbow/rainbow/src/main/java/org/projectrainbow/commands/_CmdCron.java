...
import PluginReference.MC_Player;import PluginReference.RainbowUtils;import com.google.common.io.Files;import net.minecraft.command.CommandBase;import net.minecraft.command.CommandException;import net.minecraft.command.ICommandManager;import net.minecraft.command.ICommandSender;import net.minecraft.server.MinecraftServer;import org.projectrainbow._ColorHelper;import joebkt._CronData;import org.projectrainbow._DiwUtils;import java.io.BufferedInputStream;import java.io.BufferedOutputStream;import java.io.File;import java.io.FileInputStream;import java.io.FileOutputStream;import java.io.ObjectInputStream;import java.io.ObjectOutputStream;import java.util.ArrayList;import java.util.Collections;import java.util.Comparator;import java.util.List;import java.util.Map;import java.util.concurrent.ConcurrentHashMap;public class _CmdCron extends CommandBase{
public static Map<String, _CronData> mapData;
private static String Filename;
static{_CmdCron.mapData = new ConcurrentHashMap<String, _CronData>();
...
try{final File file = new File(String.valueOf(_DiwUtils.RainbowDataDirectory) + _CmdCron.Filename);
...
}s.close();}catch (Throwable exc){System.out.println("Starting New Cron DB: " + _CmdCron.Filename);
...
try{_DiwUtils.ConsoleMsg("[CRON] Job " + data.jobName + ": " + data.cmdToRun);
...
try{final long ms = 0L;
long multi = 1L;
if (strTime.endsWith("ms")){
strTime = strTime.substring(0, strTime.length() - 2);
multi = 1L;}else if (strTime.endsWith("s")){strTime = strTime.substring(0, strTime.length() - 1);
multi = 1000L;
}else if (strTime.endsWith("m")){strTime = strTime.substring(0, strTime.length() - 1);
multi = 60000L;
}else if (strTime.endsWith("h")){strTime = strTime.substring(0, strTime.length() - 1);
multi = 3600000L;
}else if (strTime.endsWith("d")){strTime = strTime.substring(0, strTime.length() - 1);
multi = 86400000L;
}else{multi = 1000L;
}return multi * Long.parseLong(strTime);
}catch (Exception ex){return 0L;}}@Overridepublic void execute(MinecraftServer minecraftServer, ICommandSender cs, String[] args) throws CommandException{
...
}else{String jobName = args[1].trim();
...
SaveData();_DiwUtils.reply(cs, _ColorHelper.GREEN + "Saved " + _ColorHelper.AQUA + jobName);
}else{_DiwUtils.reply(cs, _ColorHelper.RED + "Usage: /cron add " + _ColorHelper.YELLOW + "MyLoop " + _ColorHelper.WHITE + "60s " + _ColorHelper.GOLD + "/give @a cake");
}}}else if (args.length >= 1 && args[0].equalsIgnoreCase("delete")){if (args.length <= 1){
...
}else{mapData.remove(key);
_DiwUtils.reply(cs, _ColorHelper.GREEN + "Deleted job: " + _ColorHelper.YELLOW + jobName);
SaveData();}}}else if (args.length >= 1 && args[0].equalsIgnoreCase("list")){if (mapData.size() <= 0){
_DiwUtils.reply(cs, _ColorHelper.AQUA + "There are no jobs setup.");
}else{_DiwUtils.reply(cs, _ColorHelper.LIGHT_PURPLE + RainbowUtils.TextLabel("Job Name", 10) + _ColorHelper.LIGHT_PURPLE + " " + RainbowUtils.TextLabel("Interval", 9) + _ColorHelper.LIGHT_PURPLE + " " + RainbowUtils.TextLabel("Time Left", 9) + _ColorHelper.LIGHT_PURPLE + " Command");
...
}}else if (args.length >= 1 && args[0].equalsIgnoreCase("setdelay")){if (args.length < 3){
...
if (ms <= 0L){
_DiwUtils.reply(cs, _ColorHelper.RED + "Usage: /cron " + _ColorHelper.AQUA + "setdelay " + _ColorHelper.YELLOW + "MyLoop " + _ColorHelper.WHITE + "5m");
}else{String key = jobName.toLowerCase();
_CronData data = (_CronData) mapData.get(key);
if (data == null){
_DiwUtils.reply(cs, _ColorHelper.RED + "No job named: " + _ColorHelper.YELLOW + jobName);
}else{data.msDelay = ms;
mapData.put(key, data);
SaveData();_DiwUtils.reply(cs, _ColorHelper.GREEN + "Updated job " + _ColorHelper.AQUA + jobName + _ColorHelper.GREEN + " new interval: " + _ColorHelper.WHITE + RainbowUtils.TimeDeltaString_JustMinutesSecs(ms));
}}}}else if (args.length >= 1 && args[0].equalsIgnoreCase("setcmd")){if (args.length < 3){
...
if (data == null){
_DiwUtils.reply(cs, _ColorHelper.RED + "No job named: " + _ColorHelper.YELLOW + jobName);
}else{data.cmdToRun = strCmd;
mapData.put(key, data);
SaveData();_DiwUtils.reply(cs, _ColorHelper.GREEN + "Updated job " + _ColorHelper.AQUA + jobName + _ColorHelper.GREEN + " new command: " + _ColorHelper.GOLD + strCmd);
}}}}else if (args.length >= 2 && args[0].equalsIgnoreCase("advance")){String strTime = RainbowUtils.ConcatArgs(args, 1).trim();
long ms = GetMSFromString(strTime);
_DiwUtils.reply(cs, _ColorHelper.GREEN + "Advancing clock: " + _ColorHelper.WHITE + _DiwUtils.TimeDeltaString_NoDays(ms));
for (String key:mapData.keySet()){
_CronData data = (_CronData) mapData.get(key);
data.msLastRun -= ms;
}}else{this.ShowUsage(cs);
}}}