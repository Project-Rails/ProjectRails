...
import PluginReference.ChatColor;import PluginReference.MC_Command;import PluginReference.MC_Player;import org.projectrainbow._DiwUtils;import org.projectrainbow._WarpManager;import java.util.List;public class _CmdDelWarp implements MC_Command{
public static String permKey = "rainbow.delwarp";
public _CmdDelWarp(){}public String getCommandName(){
return "delwarp";
...
this.SendUsage(plr);
}else{String warpName = _DiwUtils.ConcatArgs(args, 0);
if (warpName.length() <= 0){
this.SendUsage(plr);
}else{String actualWarpName = _WarpManager.GetActualWarpName(warpName);
if (actualWarpName == null){
_DiwUtils.reply(plr,
ChatColor.RED + "No warp named: " + ChatColor.YELLOW
+ warpName);
}else{_WarpManager.adminWarps.remove(actualWarpName);
_DiwUtils.reply(plr,
