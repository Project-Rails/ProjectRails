...
import PluginReference.ChatColor;import PluginReference.MC_Player;import com.google.common.io.Files;import joebkt._EmoteEntry;import net.minecraft.command.ICommandSender;import java.io.BufferedInputStream;import java.io.BufferedOutputStream;import java.io.File;import java.io.FileInputStream;import java.io.FileOutputStream;import java.io.ObjectInputStream;import java.io.ObjectOutputStream;import java.util.ArrayList;import java.util.Collections;import java.util.Map;import java.util.concurrent.ConcurrentHashMap;public class _EmoteUtils{
...
public _EmoteUtils(){}public static void LoadEmotes(){
...
}}catch (Throwable var9){var9.printStackTrace();
_DiwUtils.ConsoleMsg("Starting New Emote Database...");
emotes = new ConcurrentHashMap<String, _EmoteEntry>();
...
if (cmd == null){
return false;
}else if (cmd.length() <= 0){return false;
}else{String[] pieces = cmd.split("\\s+");
String emote = pieces[0].toLowerCase();
if (!emotes.containsKey(emote)){
return false;
}else if (!CanDoEmote(cs, emote)){_DiwUtils.reply(cs, ChatColor.RED + "You don\'t yet have this emote! :(...");
return true;
}else if (_DiwUtils.TooSoon((ICommandSender) cs, "Emote", 7)){return true;
}else{_EmoteEntry entry = emotes.get(emote);
if (pieces.length > 1){
...
return true;
}else{MC_Player pTgt = ServerWrapper.getInstance().getOnlinePlayerByName(tgtName);
if (pTgt == null){
_DiwUtils.reply(cs, ChatColor.RED + "Target player not found: " + ChatColor.YELLOW + tgtName);
return true;
}else{String trailer = String.format(entry.msg.get("other"), pTgt.getName());
_DiwUtils.MessageAllPlayers(cs, ChatColor.GREEN + cs.getName() + " " + trailer);
return true;
}}}else{_DiwUtils.MessageAllPlayers(cs, ChatColor.GREEN + cs.getName() + " " + entry.msg.get("default"));
return true;
...
return true;
}else if (args.length >= 1 && args[0].equalsIgnoreCase("mine")){ArrayList<String> emote1 = new ArrayList<String>(emotes.keySet());
new StringBuffer();
