...
import PluginReference.RainbowUtils;import net.minecraft.entity.Entity;import net.minecraft.entity.EntityList;import net.minecraft.entity.player.EntityPlayer;import net.minecraft.entity.player.EntityPlayerMP;import net.minecraft.world.WorldServer;import org.projectrainbow.interfaces.IMixinICommandSender;import java.util.ArrayList;import java.util.Collections;import java.util.Comparator;import java.util.HashMap;import java.util.List;import java.util.Map;public class _Janitor{
...
++nPlayers;}else{int mobX = (int) ent.posX;
...
shouldRemove = true;
break;}if (cleanTarget[idx].equalsIgnoreCase(mobName) || cleanTarget[idx].equals("*") || mobName.startsWith("item.") && cleanTarget[idx].equalsIgnoreCase("item")){
shouldRemove = true;
break;}}}for (Entity entity:ent.getPassengers()){
if (entity instanceof EntityPlayer){
shouldRemove = false;
}}if (shouldRemove){
++nRemoved;ent.setDead();
}}if (!isPlayer && !ent.isDead){
Integer cnt = mobCounts.get(mobName);
if (cnt == null){
cnt = 0;}mobCounts.put(mobName, cnt + 1);
...
Integer cnt = mobCounts.get(mobName);
tot += cnt;String msg = String.format("%4d %s", cnt, mobName);
System.out.println(msg);
if (p != null){
((IMixinICommandSender) p).sendMessage(_ColorHelper.LIGHT_PURPLE + msg);
}}System.out.println(dashes);
