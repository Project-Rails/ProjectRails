...
import com.google.common.io.Files;import joebkt._SerializableLocation;import java.io.File;import java.io.FileInputStream;import java.io.FileOutputStream;import java.io.ObjectInputStream;import java.io.ObjectOutputStream;import java.util.Map;import java.util.UUID;import java.util.concurrent.ConcurrentHashMap;public class _HomeUtils{
...
static{_HomeUtils.playerHomes = new ConcurrentHashMap<String, _SerializableLocation>();
...
try{final File file = new File(String.valueOf(_DiwUtils.RainbowDataDirectory) + _HomeUtils.DataFilename);
...
return;}final FileInputStream f = new FileInputStream(file);
...
s.close();final String msg = String.format("Homes Loaded: " + _ColorHelper.WHITE + "%d homes.", _HomeUtils.playerHomes.size());
_DiwUtils.ConsoleMsg(msg);
...
try{final File file = new File(String.valueOf(_DiwUtils.RainbowDataDirectory) + _HomeUtils.DataFilename2);
...
s.close();final String msg = String.format("Homes Loaded: " + _ColorHelper.WHITE + "%d homes.", _HomeUtils.playerHomes2.size());
_DiwUtils.ConsoleMsg(msg);
}catch (Throwable exc){exc.printStackTrace();
System.out.println("Starting New Home2 Database...");
_HomeUtils.playerHomes2 = new ConcurrentHashMap<String, _SerializableLocation>();
