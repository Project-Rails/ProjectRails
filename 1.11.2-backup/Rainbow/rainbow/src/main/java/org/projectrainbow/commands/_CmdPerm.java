...
import PluginReference.ChatColor;import PluginReference.MC_Command;import PluginReference.MC_Player;import net.minecraft.command.CommandBase;import org.projectrainbow._DiwUtils;import org.projectrainbow._PermMgr;import org.projectrainbow._UUIDMapper;import java.util.List;import java.util.UUID;public class _CmdPerm implements MC_Command{
...
if (args.length == 0){
this.showUsage(plr);
}else if (args.length == 1 && args[0].equalsIgnoreCase("debug")){DebugMode = !DebugMode;
_DiwUtils.reply(plr,
...
return;}_PermMgr.givePermission(uuid, perms1);
_DiwUtils.reply(plr,
ChatColor.GREEN
+ String.format("Gave \'%s\' Permission \'%s\'.",
name, perms1));
}else if (args.length == 3 && args[0].equalsIgnoreCase("take")){name = args[1];
perms1 = args[2];
UUID uuid = _UUIDMapper.getUUID(name);
if (uuid == null && !name.equals("*")){
_DiwUtils.reply(plr, ChatColor.RED + String.format("Player \'%s\' not found.", name));
return;}if (!_PermMgr.hasPermission(uuid, perms1)){
_DiwUtils.reply(plr,
ChatColor.GREEN
+ String.format(
"Permission \'%s\' is not granted to \'%s\'.",
perms1, name));
}else{_PermMgr.takePermission(uuid, perms1);
_DiwUtils.reply(plr,
ChatColor.GREEN
+ String.format(
"Took Permission \'%s\' from \'%s\'.",
perms1, name));
}}else if (args.length == 3 && args[0].equalsIgnoreCase("test")){name = args[1];
perms1 = args[2];
UUID uuid = _UUIDMapper.getUUID(name);
if (uuid == null && !name.equals("*")){
_DiwUtils.reply(plr, ChatColor.RED + String.format("Player \'%s\' not found.", name));
...
name, perms1));
}}else if (args.length == 2 && args[0].equalsIgnoreCase("list")){name = args[1];
UUID uuid = _UUIDMapper.getUUID(name);
if (uuid == null && !name.equals("*")){
_DiwUtils.reply(plr, ChatColor.RED + String.format("Player \'%s\' not found.", name));
return;}List perms = _PermMgr.getPermissions(uuid);
...
: null;}public void showUsage(MC_Player plr){
_DiwUtils.reply(plr,
