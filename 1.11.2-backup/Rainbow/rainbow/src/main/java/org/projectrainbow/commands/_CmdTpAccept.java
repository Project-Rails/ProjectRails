...
import PluginReference.ChatColor;import PluginReference.MC_Command;import PluginReference.MC_Location;import PluginReference.MC_Player;import net.minecraft.command.CommandBase;import org.projectrainbow._DiwUtils;import java.util.Collections;import java.util.List;public class _CmdTpAccept implements MC_Command{
public _CmdTpAccept(){
...
}else{String key = pName.toLowerCase() + "."
+ tgtName.toLowerCase();
...
}else if (player.getLocation().dimension != sloc.dimension){player.sendMessage(
ChatColor.RED + "You must be in the same world.");
}else{_CmdTpaHere.tpahereMap.remove(key);
...
: null;}public String getCommandName(){
return "tpaccept";
}}