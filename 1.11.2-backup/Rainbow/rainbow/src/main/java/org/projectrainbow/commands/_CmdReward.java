...
import PluginReference.ChatColor;import PluginReference.MC_Command;import PluginReference.MC_ItemStack;import PluginReference.MC_Player;import org.projectrainbow._DiwUtils;import org.projectrainbow._DynReward;import java.util.List;public class _CmdReward implements MC_Command{
...
this.SendHelp(plr);
}else if (args[0].equalsIgnoreCase("delete")){_DynReward.CmdDelete(plr);
}else if (args[0].equalsIgnoreCase("set")){_DynReward.CmdSet(plr, args);
}else if (args[0].equalsIgnoreCase("list")){_DynReward.CmdList(plr);
}else if (args[0].equalsIgnoreCase("iteminfo")){MC_ItemStack is = plr.getItemInHand();
plr.sendMessage(
