...
import PluginReference.*;import com.google.common.base.Function;import com.google.common.collect.Lists;import com.google.common.collect.Maps;import net.minecraft.block.Block;import net.minecraft.command.ICommand;import net.minecraft.command.ServerCommandManager;import net.minecraft.entity.ai.attributes.AttributeModifier;import net.minecraft.entity.player.EntityPlayer;import net.minecraft.entity.player.EntityPlayerMP;import net.minecraft.item.Item;import net.minecraft.item.ItemStack;import net.minecraft.item.crafting.CraftingManager;import net.minecraft.item.crafting.IRecipe;import net.minecraft.item.crafting.ShapedRecipes;import net.minecraft.item.crafting.ShapelessRecipes;import net.minecraft.nbt.NBTTagCompound;import org.projectrainbow.commands._CmdPerm;import org.projectrainbow.interfaces.IMixinMinecraftServer;import org.projectrainbow.interfaces.IMixinNBTBase;import org.projectrainbow.plugins.PluginCommand;import org.projectrainbow.util.WrappedMinecraftCommand;import javax.annotation.Nullable;import java.io.ByteArrayInputStream;import java.io.DataInputStream;import java.util.*;public class ServerWrapper implements MC_Server{
...
try{_DiwUtils.getMinecraftServer().getCommandManager().executeCommand(_DiwUtils.getMinecraftServer(), cmd);
}catch (Exception var3){;}}public List<PluginInfo> getPlugins(){
...
}catch (Throwable th){uuid = _UUIDMapper.getUUID(key);
}if (uuid == null){
return;}}_PermMgr.givePermission(uuid, perm);
...
return;}}_PermMgr.takePermission(uuid, perm);
...
}return res;}public MC_World getWorld(int idxDimension){
...
return plrName == null
? null: _JOT_OnlineTimeUtils.GetPlayerExactName(plrName);
...
if (pName == null){
return null;
}else{pName = pName.trim();
if (pName.length() <= 0){
return null;
}else{return (MC_Player) _DiwUtils.getMinecraftServer().getPlayerList().getPlayerByUsername(pName);
}}}@Overridepublic MC_Player getOnlinePlayerByUUID(UUID uuid){
if (uuid == null){
return null;
}else{return (MC_Player) _DiwUtils.getMinecraftServer().getPlayerList().getPlayerByUUID(uuid);
...
}});}@Overridepublic void addShapelessRecipe(MC_ItemStack result, MC_ItemStack... ingredients){
...
*/@Overridepublic MC_AttributeModifier createAttributeModifier(String name, MC_AttributeModifier.Operator operator, double value){
...
* @param value    the value of the modifier
* @return the created attribute modifier
*/@Overridepublic MC_AttributeModifier createAttributeModifier(UUID uuid, String name, MC_AttributeModifier.Operator operator, double value){
return (MC_AttributeModifier) new AttributeModifier(uuid, name, value, PluginHelper.operatorMap.get(operator));
}@Overridepublic MC_InventoryGUI createInventoryGUI(int size, String title){
if (size < 0){
...
}@Overridepublic MC_Player getOfflinePlayerByName(String name){
...
}@Overridepublic MC_Player getOfflinePlayerByUUID(UUID uuid){
...
}@Overridepublic int getMaxPlayerCount(){
return _DiwUtils.getMinecraftServer().getPlayerList().getMaxPlayers();
}@Overridepublic String getMinecraftVersion(){
