...
import PluginReference.*;import com.google.common.base.Objects;import com.google.common.collect.BiMap;import com.google.common.collect.HashBiMap;import net.minecraft.block.Block;import net.minecraft.entity.*;import net.minecraft.entity.ai.attributes.IAttribute;import net.minecraft.entity.boss.EntityDragon;import net.minecraft.entity.boss.EntityWither;import net.minecraft.entity.effect.EntityLightningBolt;import net.minecraft.entity.item.*;import net.minecraft.entity.monster.*;import net.minecraft.entity.passive.*;import net.minecraft.entity.player.EntityPlayerMP;import net.minecraft.entity.projectile.*;import net.minecraft.init.MobEffects;import net.minecraft.item.ItemStack;import net.minecraft.potion.Potion;import net.minecraft.potion.PotionEffect;import net.minecraft.util.DamageSource;import net.minecraft.util.EnumFacing;import net.minecraft.util.EnumHand;import net.minecraft.world.Explosion;import net.minecraft.world.GameType;import net.minecraft.world.biome.Biome;import java.util.ArrayList;import java.util.Iterator;import java.util.List;import java.util.Map;public class PluginHelper{
...
}else{return Objects.firstNonNull(entityMap.get(clazz), MC_EntityType.UNSPECIFIED);
...
}static{directionMap.put(EnumFacing.DOWN, MC_DirectionNESWUD.DOWN);
...
if (blockName == null){
return null;
}else if (blockName.length() <= 0){return null;
}else{if (blockName.equalsIgnoreCase("white")){
blockName = "wool";
}Block bo = Block.getBlockFromName(blockName.toLowerCase());
if (bo != null){
return new BlockWrapper(bo.getDefaultState());
}else{Iterator blkID = BlockHelper.mapItemNames.entrySet().iterator();
while (blkID.hasNext()){
...
break;}while (subType.contains(" ")){
subType = subType.replace(" ", "_");
}if (subType.equalsIgnoreCase(blockName)
|| subType.equalsIgnoreCase(blockName + "_wool")){
blockName = (String) idxColon.getKey();
break;}}int idxColon1 = blockName.indexOf(58);
boolean blkID1 = true;
int subType1 = 0;
String realBlockName;
int blkID2;if (idxColon1 < 0){
try{blkID2 = Integer.parseInt(blockName);
}catch (Exception var8){blkID2 = -1;
}}else{try{realBlockName = blockName.substring(0, idxColon1);
String bw = blockName.substring(idxColon1 + 1);
blkID2 = Integer.parseInt(realBlockName);
subType1 = Integer.parseInt(bw);
}catch (Exception var7){blkID2 = -1;
subType1 = -1;
}}if (blkID2 < 0){
return null;
}else if (subType1 < 0){return null;
}else{realBlockName = (String) BlockHelper.mapBlockNames.get(
...
}return bw1;}}return null;
}}}}public static MC_DamageType wrap(DamageSource damageSource){
if (damageSource == null){
return MC_DamageType.UNSPECIFIED;
}else if (damageSource == DamageSource.anvil){return MC_DamageType.ANVIL;
}else if (damageSource == DamageSource.cactus){return MC_DamageType.CACTUS;
}else if (damageSource == DamageSource.drown){return MC_DamageType.DROWN;
}else if (damageSource == DamageSource.fall){return MC_DamageType.FALL;
}else if (damageSource == DamageSource.fallingBlock){return MC_DamageType.FALLING_BLOCK;
}else if (damageSource == DamageSource.generic){return MC_DamageType.GENERIC;
}else if (damageSource == DamageSource.inFire){return MC_DamageType.IN_FIRE;
}else if (damageSource == DamageSource.inWall){return MC_DamageType.IN_WALL;
}else if (damageSource == DamageSource.lava){return MC_DamageType.LAVA;
}else if (damageSource == DamageSource.lightningBolt){return MC_DamageType.LIGHTING_BOLT;
}else if (damageSource == DamageSource.magic){return MC_DamageType.MAGIC;
}else if (damageSource == DamageSource.onFire){return MC_DamageType.ON_FIRE;
}else if (damageSource == DamageSource.outOfWorld){return MC_DamageType.OUT_OF_WORLD;
}else if (damageSource == DamageSource.starve){return MC_DamageType.STARVE;
}else if (damageSource == DamageSource.wither){return MC_DamageType.WITHER;
}else if (damageSource == DamageSource.dragonBreath){return MC_DamageType.DRAGON_BREATH;
}else if (damageSource == DamageSource.hotFloor){return MC_DamageType.HOT_FLOOR;
}else{if (damageSource.getDamageType() != null){
