...
import PluginReference.ChatColor;import PluginReference.MC_Player;import com.google.common.io.Files;import java.io.BufferedReader;import java.io.File;import java.io.FileReader;import java.util.ArrayList;public class _Announcer{
...
public _Announcer(){}public static void LoadAnnouncements(){
try{Seconds = 60;
...
IsEnabled = false;
}}else if (lwr.startsWith("prefix=")){AnnouncerPrefix = _DiwUtils.TranslateColorString(
line.substring(7), true);
}else{line = _DiwUtils.TranslateColorString(line, true);
announcements.add(line);
}}}}br.close();}catch (Exception var5){_DiwUtils.ConsoleMsg("No Announcements loaded.");
...
}else if (args[0].equalsIgnoreCase("forcenext")){CurrentSecondCount = Seconds;
_DiwUtils.reply(cs, ChatColor.GREEN + "OK.");
}else{int idx;if (!args[0].equalsIgnoreCase("list")){
if (args[0].equalsIgnoreCase("broadcast")){
if (args.length <= 1){
_DiwUtils.reply(cs,
ChatColor.RED + "Usage: /announcer broadcast #");
}else{idx = -1;try{idx = Integer.parseInt(args[1]);
}catch (Throwable var4){;}--idx;if (idx >= 0 && idx < announcements.size()){
BroadcastIdx(idx);
}else{_DiwUtils.reply(cs,
ChatColor.RED
...
}}else{for (idx = 0; idx < announcements.size(); ++idx){
_DiwUtils.reply(cs,
ChatColor.WHITE
+ String.format(
...
announcements.get(idx)}));}_DiwUtils.reply(cs,
ChatColor.GREEN + "Number of Announcements: "
+ ChatColor.WHITE + announcements.size());
...
try{BroadcastIdx(idx);
}catch (Throwable var3){_DiwUtils.ConsoleMsg(
