...
import PluginReference.*;import org.apache.logging.log4j.LogManager;import org.apache.logging.log4j.Logger;import java.net.InetAddress;import java.util.List;import java.util.UUID;public class Hooks{
private static Logger logger = LogManager.getLogger("Minecraft");
public static void onShutdown(){
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onShutdown();
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
}}}public static void onTick(int tickNumber){
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onTick(tickNumber);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
...
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onAttemptExplosion(loc, ei);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
}}}public static boolean onAttemptExplodeSpecific(MC_Entity ent, List<MC_Location> locs){
boolean result = false;
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{result |= plugin.ref.onAttemptExplodeSpecific(ent, locs);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
}}return result;
...
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onAttemptDamageHangingEntity(plr, loc, entType, ei);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
...
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onAttemptBlockPlace(plr, loc, blk, isHandItem, locPlacedAgainst, dir, ei);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
}}}public static void onAttemptDeath(MC_Entity entVictim, MC_Entity entKiller, MC_DamageType dmgType, float dmgAmount){
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onAttemptDeath(entVictim, entKiller, dmgType, dmgAmount);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
...
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onAttemptProjectileHitEntity(projectile, entity, hitLocation, ei);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
...
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onAttemptProjectileHitBlock(projectile, blockLocation, face, hitLocation, ei);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
}}}public static void onPlayerKick(MC_Player player, String reason, MC_EventInfo ei){
for (PluginInfo plugin:_DiwUtils.pluginManager.plugins){
try{plugin.ref.onPlayerKick(player, reason, ei);
}catch (Throwable th){logger.error("Failed to pass event to plugin " + plugin.name, th);
}}}}