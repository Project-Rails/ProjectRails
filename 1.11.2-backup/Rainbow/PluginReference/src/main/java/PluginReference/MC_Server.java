...
import java.util.List;import java.util.Map;import java.util.UUID;/*** Represents the Minecraft Server
...
*/@Deprecatedvoid registerPlayerPacketListener(MC_PlayerPacketListener myListener);
...
*/@Deprecatedvoid registerServerPacketListener(MC_ServerPacketListener myListener);
...
*/@Deprecatedint registerWorld(String worldName, MC_WorldSettings settings);
/*** Unregister a world
** @param worldName World Name
* @return True if unregistered, False if unrecognized
*/@Deprecatedboolean unregisterWorld(String worldName);
/*** Get list of world objects
** @return List of MC_World objects
*/List<MC_World> getWorlds();
/*** Add a shaped recipe
* <p>* e.g the following code sample adds a recipe for crafting a stone pickaxe
* using clean stone.
* <pre>*{@code* MC_Server server = RainbowUtils.getServer();
...
* }* </pre>** @param result      craft result
* @param ingredients recipe
*/void addRecipe(MC_ItemStack result, Object... ingredients);
/*** Add a shapeless recipe.
* <p>* e.g the following code sample adds a recipe that turns rotten flesh into
* leather.* <pre>*{@code* MC_Server server = RainbowUtils.getServer();
...
* @return the{@link MC_InventoryGUI} instance*/MC_InventoryGUI createInventoryGUI(int size, String title);
/*** Get an offline player by the name. If the player is online this method returns the same object as
*{@link #getOnlinePlayerByName(String)}.** Also look at the javadoc of{@link MC_Player} to find out which methods are implemented for offline players.** @param name the name of the player
...
*{@link #getOnlinePlayerByUUID(UUID)}.** Also look at the javadoc of{@link MC_Player} to find out which methods are implemented for offline players.** @param uuid the uuid of the player
* @return the (offline) player if present
