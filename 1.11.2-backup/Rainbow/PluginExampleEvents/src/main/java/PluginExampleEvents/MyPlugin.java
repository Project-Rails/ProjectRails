...
import PluginReference.BlockHelper;import PluginReference.ChatColor;import PluginReference.MC_Block;import PluginReference.MC_Chest;import PluginReference.MC_Container;import PluginReference.MC_ContainerType;import PluginReference.MC_DamageType;import PluginReference.MC_DirectionNESWUD;import PluginReference.MC_Entity;import PluginReference.MC_EventInfo;import PluginReference.MC_Hand;import PluginReference.MC_HangingEntityType;import PluginReference.MC_ItemFrameActionType;import PluginReference.MC_ItemStack;import PluginReference.MC_ItemType;import PluginReference.MC_Location;import PluginReference.MC_MiscGriefType;import PluginReference.MC_Player;import PluginReference.MC_PotionEffectType;import PluginReference.MC_PrimedTNT;import PluginReference.MC_Projectile;import PluginReference.MC_Server;import PluginReference.MC_Sign;import PluginReference.MC_Skeleton;import PluginReference.PluginBase;import PluginReference.PluginInfo;import PluginReference.RainbowUtils;import java.net.InetAddress;import java.util.ArrayList;import java.util.List;import java.util.UUID;public class MyPlugin extends PluginBase{
public static MC_Server server = null;
@Overridepublic void onStartup(MC_Server argServer){
System.out.println("======= Event Sample --- Startup! =======");
server = argServer;
}@Overridepublic void onShutdown(){
System.out.println("======= Event Sample --- Shutdown! =======");
}@Overridepublic PluginInfo getPluginInfo(){
...
@Overridepublic void onConsoleInput(String cmd, MC_EventInfo ei){
...
System.out.println("---------------------");
return;}}@Overridepublic void onPlayerInput(MC_Player plr, String msg, MC_EventInfo ei){
if (ei.isCancelled) return;
...
return;}if (msg.equalsIgnoreCase("/TogglePlace")){
ei.isCancelled = true;
...
return;}if (msg.equalsIgnoreCase("/ToggleBreak")){
ei.isCancelled = true;
...
return;}if (msg.equalsIgnoreCase("/ToggleExplode")){
ei.isCancelled = true;
...
return;}if (msg.equalsIgnoreCase("/ToggleHanging")){
ei.isCancelled = true;
...
return;}if (msg.startsWith("/bedforce")){
ei.isCancelled = true;
MC_Location loc = plr.getLocation();
plr.setBedRespawnLocation(loc, false);
plr.sendMessage(ChatColor.GREEN + "Bed Respawn Location Set To: " + ChatColor.WHITE + plr.getBedRespawnLocation().toString());
return;}if (msg.startsWith("/bedclear")){
ei.isCancelled = true;
plr.setBedRespawnLocation(null, false);
plr.sendMessage(ChatColor.GREEN + "Bed Cleared");
return;}}@Overridepublic void onPlayerLogin(String playerName, UUID uuid, InetAddress address, MC_EventInfo ei){
...
}@Overridepublic void onPlayerJoin(MC_Player plr){
...
plr.updateInventory();
}}@Overridepublic void onPlayerLogout(String playerName, UUID uuid){
String logMsg = String.format("%s LOGOUT. UUID=%s", playerName, uuid.toString());
System.out.println("EventSamplePlugin -- " + logMsg);
} Past Tense / "After" Events (i.e. can't cancel)
 ------------------------------------------------------------------------
@Overridepublic void onItemPlaced(MC_Player plr, MC_Location loc, MC_ItemStack isHandItem, MC_Location locPlacedAgainst, MC_DirectionNESWUD dir){
...
@Overridepublic void onInteracted(MC_Player plr, MC_Location loc, MC_ItemStack isHandItem){
...
nextDir++;if (nextDir >= dirVals.length) nextDir = 1;
plr.getWorld().setBlockFacing((int) loc.x, (int) loc.y, (int) loc.z, newDir);
System.out.println("========= ARROW: Adjusted Direction: " + newDir);
}}@Overridepublic void onBlockBroke(MC_Player plr, MC_Location loc, MC_Block blk){
...
}@Overridepublic void onPlayerDeath(MC_Player plrVictim, MC_Player plrKiller, MC_DamageType dmgType, String deathMsg){
...
System.out.println("EventSamplePlugin -- " + logMsg);
}@Overridepublic void onPlayerRespawn(MC_Player plr){
String logMsg = String.format("%s RESPAWNED TO %s", plr.getName(), plr.getLocation().toString());
System.out.println("EventSamplePlugin -- " + logMsg);
} ------------------------------------------------------------------------
 Events you can cancel...
 ------------------------------------------------------------------------
@Overridepublic void onAttemptBlockBreak(MC_Player plr, MC_Location loc, MC_EventInfo ei){
...
}}@Overridepublic void onAttemptPlaceOrInteract(MC_Player plr, MC_Location loc, MC_DirectionNESWUD dir, MC_Hand hand, MC_EventInfo ei){
...
elseSystem.out.println("* This chest is all alone! He needs a hug...");
} end of 'if chest'MC_Container container = plr.getWorld().getContainerAt(loc);
if (container != null){
...
}@Overridepublic void onAttemptExplosion(MC_Location loc, MC_EventInfo ei){
if (ei.isCancelled) return;
...
@Overridepublic void onAttemptDamageHangingEntity(MC_Player plr, MC_Location loc, MC_HangingEntityType entType, MC_EventInfo ei){
if (ei.isCancelled) return;
...
}}@Overridepublic void onAttemptItemFrameInteract(MC_Player plr, MC_Location loc, MC_ItemFrameActionType actionType, MC_EventInfo ei){
if (ei.isCancelled) return;
 Player could be null (like if damage from explosion)
String strName = (plr == null) ? "?" : plr.getName();
...
ei.isCancelled = true;
System.out.println("Prevented: " + logMsg);
}}@Overridepublic void onAttemptPotionEffect(MC_Player plr, MC_PotionEffectType potionType, MC_EventInfo ei){
if (ei.isCancelled) return;
...
ei.isCancelled = true;
System.out.println("Prevented: " + logMsg);
}}@Overridepublic void onAttemptPlayerTeleport(MC_Player plr, MC_Location loc, MC_EventInfo ei){
if (ei.isCancelled) return;
...
@Overridepublic void onAttemptPlayerChangeDimension(MC_Player plr, int newDimension, MC_EventInfo ei){
if (ei.isCancelled) return;
...
ei.isCancelled = true;
System.out.println("Prevented: " + logMsg);
}}@Overridepublic void onAttemptItemDrop(MC_Player plr, MC_ItemStack is, MC_EventInfo ei){
...
@Overridepublic void onAttemptAttackEntity(MC_Player plr, MC_Entity ent, MC_EventInfo ei){
...
entName);System.out.println("EventSamplePlugin -- " + logMsg);
if (ToggleAttack){
...
@Overridepublic void onAttemptEntityDamage(MC_Entity ent, MC_DamageType dmgType, double amt, MC_EventInfo ei){
String entName = ent.getName();
...
}}@Overridepublic void onAttemptPistonAction(MC_Location loc, MC_DirectionNESWUD dir, MC_EventInfo ei){
String logMsg = String.format("onAttemptPistonAction: %s w/direction %s", loc.toString(), dir.toString());
System.out.println("EventSamplePlugin -- " + logMsg);
if (TogglePistons){
ei.isCancelled = true;
System.out.println("Prevented: " + logMsg);
}}@Overridepublic void onAttemptBlockFlow(MC_Location loc, MC_Block blk, MC_EventInfo ei){
String logMsg = String.format("onFlow: %s w/blk id %d", loc.toString(), blk.getId());
System.out.println("EventSamplePlugin -- " + logMsg);
if (ToggleFlow){
ei.isCancelled = true;
System.out.println("Prevented: " + logMsg);
}}@Overridepublic void onContainerOpen(MC_Player plr, List<MC_ItemStack> items, String internalClassName){
...
@Overridepublic void onAttemptPlayerMove(MC_Player plr, MC_Location locFrom, MC_Location locTo, MC_EventInfo ei){
...
ei.isCancelled = true;
}}@Overridepublic void onPacketSoundEffect(MC_Player plr, String soundName, MC_Location loc, MC_EventInfo ei){
String logMsg = String.format("onPacketSoundEffect: %s: %s @ %s", plr.getName(), soundName, loc.toString());
System.out.println("EventSamplePlugin -- " + logMsg);
if (ToggleSound){
ei.isCancelled = true;
}}@Overridepublic void onSignChanging(MC_Player plr, MC_Sign sign, MC_Location loc, List<String> newLines, MC_EventInfo ei){
...
elseSystem.out.println("SIGN COORDINATES WRONG?  No Sign at location.");
}@Overridepublic void onSignChanged(MC_Player plr, MC_Sign sign, MC_Location loc){
...
if (signVerify != null) System.out.println("SIGN COORDINATES VERIFIED");
elseSystem.out.println("SIGN COORDINATES WRONG?  No Sign at location.");
}@Overridepublic void onAttemptEntitySpawn(MC_Entity ent, MC_EventInfo ei){
...
ei.isCancelled = true;
}}@Overridepublic void onAttemptHopperReceivingItem(MC_Location loc, MC_ItemStack is, boolean isMinecartHopper, MC_EventInfo ei){
...
System.out.println("EventSamplePlugin -- " + logMsg);
}@Overridepublic void onAttemptBookChange(MC_Player plr, List<String> bookContent, MC_EventInfo ei){
...
}@Overridepublic boolean onAttemptExplodeSpecific(MC_Entity ent, List<MC_Location> locs){
String logMsg = "onAttemptExplodeSpecific ~ " + locs.get(0).toString();
System.out.println("EventSamplePlugin -- " + logMsg);
return false;
}@Overridepublic void onAttemptCropTrample(MC_Entity ent, MC_Location loc, MC_EventInfo ei){
String logMsg = String.format("onAttemptCropTrample: %s (%s) @ %s", ent.getName(), ent.getType(), loc.toString());
System.out.println("EventSamplePlugin -- " + logMsg);
}@Overridepublic void onFallComplete(MC_Entity ent, float fallDistance, MC_Location loc, boolean isWaterLanding){
...
}}@Overridepublic void onNonPlayerEntityDeath(MC_Entity entVictim, MC_Entity entKiller, MC_DamageType dmgType){
String strKiller = dmgType.toString();
...
System.out.println("EventSamplePlugin -- " + logMsg);
}@Overridepublic void onAttemptItemUse(MC_Player plr, MC_ItemStack is, MC_EventInfo ei){
String logMsg = String.format("onAttemptItemUse: %s : %s", plr.getName(), is.getFriendlyName());
System.out.println("EventSamplePlugin -- " + logMsg);
if (ToggleUse){
ei.isCancelled = true;
System.out.println("-- Cancelling Item Use!");
}}@Overridepublic Boolean onRequestPermission(String playerKey, String permission){
...
return null;
}@Overridepublic void onContainerClosed(MC_Player plr, MC_ContainerType containerType){
System.out.println(String.format("EventSamplePlugin -- onContainerClosed: %s closed %s", plr.getName(), containerType + ""));
}@Overridepublic void onAttemptItemPickup(MC_Player plr, MC_ItemStack is, boolean isXpOrb, MC_EventInfo ei){
...
if (TogglePickup){
ei.isCancelled = true;
}}@Overridepublic void onAttemptEntityInteract(MC_Player plr, MC_Entity ent, MC_EventInfo ei){
...
}@Overridepublic void onPlayerBedEnter(MC_Player plr, MC_Block bedBlk, MC_Location bedLoc){
...
}@Overridepublic void onPlayerBedLeave(MC_Player plr, MC_Block blk, MC_Location bedLoc){
...
}@Overridepublic void onItemCrafted(MC_Player plr, MC_ItemStack isCraftedItem){
...
}}@Overridepublic void onAttemptBlockPlace(MC_Player plr, MC_Location loc, MC_Block blk, MC_ItemStack isHandItem, MC_Location locPlacedAgainst, MC_DirectionNESWUD dir, MC_EventInfo ei){
...
if (ToggleBlockPlace){
ei.isCancelled = true;
}}@Overridepublic void onServerFullyLoaded(){
System.out.println("EventSamplePlugin -- onServerFullyLoaded");
}@Overridepublic void onAttemptDeath(MC_Entity entVictim, MC_Entity entKiller, MC_DamageType dmgType, float dmgAmount){
String strKiller = "?";
if (entKiller != null)
...
}}@Overridepublic void onAttemptFishingReel(MC_Player plr, MC_ItemStack isCatch, MC_Entity entCatch, boolean groundCatch, MC_EventInfo ei){
String strCatchType = "?";
if (isCatch != null){
System.out.println(String.format("EventSample: onAttemptFishingReel %s caught %s", plr.getName(), isCatch.getFriendlyName()));
}else if (entCatch != null){System.out.println(String.format("EventSample: onAttemptFishingReel %s snared entity %s", plr.getName(), entCatch.getName()));
}else if (groundCatch){System.out.println(String.format("EventSample: onAttemptFishingReel %s caught the ground!", plr.getName()));
...
}}@Overridepublic void onAttemptEntityMiscGrief(MC_Entity ent, MC_Location loc, MC_MiscGriefType griefType, MC_EventInfo ei){
...
}}@Overridepublic void onAttemptSpectateEntity(MC_Player plr, MC_Entity ent, MC_EventInfo ei){
String strEnt = "NONE";
...
}}@Overridepublic void onAttemptDispense(MC_Location loc, int idxItem, MC_Container container, MC_EventInfo ei){
...
}}@Overridepublic void onAttemptProjectileHitEntity(MC_Projectile projectile, MC_Entity entity, MC_Location hitLocation, MC_EventInfo ei){
...
System.out.println("EventSamplePlugin -- " + logMsg);
}@Overridepublic void onAttemptProjectileHitBlock(MC_Projectile projectile, MC_Location blockLocation, MC_DirectionNESWUD blockFaceHit, MC_Location hitLocation, MC_EventInfo ei){
...
System.out.println("EventSamplePlugin -- " + logMsg);
}@Overridepublic void onPlayerKick(MC_Player player, String reason, MC_EventInfo ei){
